...
const ElectronicsItem = ({
  ...
  const checkWishlistStatus = async () => {
    try {
      ...
      const response = await axios.get(`https://server-kzwj.onrender.com/api/wishlist/check/${productId}`, {
        headers: {
  ...
  // Handle add to cart - UPDATED FUNCTION
      ...
      <div className="relative bg-gray-50 overflow-hidden aspect-[4/5]">
        ...
        <div className="absolute bottom-4 left-4 right-4 z-20">
          <button
            ...
            className={`w-full text-white py-3 text-sm font-medium tracking-wide 
            opacity-0 translate-y-4 
            group-hover:opacity-100 group-hover:translate-y-0 
            transition-all duration-300 delay-150 flex items-center justify-center
            ${isAddingToCart ? 'bg-gray-600' : 'bg-black hover:bg-gray-800'}`}
          >
            {isAddingToCart ? (
              <>
